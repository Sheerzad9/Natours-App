...
exports.getTour = catchAsync(async (req, res, next) => {
  // 1) get the data, for requested tour(including the reviews and tour guides)
...
exports.getMyTours = catchAsync(async (req, res, next) => {
  ...
  // Below we select all the tours which have the same id that we have in our "tourIDs" array
...
exports.updateUserData = catchAsync(async (req, res, next) => {
  const updatedUser = await User.findByIdAndUpdate(
    req.user.id,
    {
      // Notice that we use "name:" and "email:" fields to update, because if we would pass directly req.body it would create vulnerabilities to our application, this we way we control what fields will be updated.
      name: req.body.name,
